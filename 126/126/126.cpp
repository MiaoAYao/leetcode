...
bool search(Node* n, string &endWord, vector<string> &wordList, vector<int> &flag,
	                 vector<Node*> &wordTree, int flagNum, map<int, Node*> &numToNode)
{
	vector<int> temp;
	for (int i = 0; i < wordList.size(); ++i)
	{
		//该节点在该层以上未被使用过
		if (flag[i] == 0 || flag[i] == flagNum)
		{
			int temp;
			temp = compare(n->name, wordList[i]);
			if (temp == 1)
			{
				...
				if (flag[i] == 0)
				{
					...
					tempNode->name = wordList[i];
					tempNode->parents.push_back(n);

					wordTree.push_back(tempNode);

					numToNode.insert(pair<int, Node*>(i, tempNode));

					flag[i] = flagNum;
				}
				//该节点在该层被使用过
				else if (flag[i] == flagNum)
				{
					//之后就不会考虑该节点的子节点
					//一定可以找到
					numToNode.find(i)->second->parents.push_back(n);
				}
				
			}
			else if (temp == 0)
			{
				//只有该单词与beginWord相同时才会出现这种情况
				flag[i] = -1;
			}
		}
	}
...
void reverse(vector<string> &partResult)
{
	//再将partResult倒序
	int size = partResult.size();
	for (int i = size - 1; i > -1; --i)
	{
		partResult.push_back(partResult[i]);
	}
	partResult.erase(partResult.begin(), partResult.begin() + size);
}

void traverse(Node* n, vector<vector<string>> &result)
{
	vector<string> partialPath;
	Node* tempNode = n;
	stack<Node*> s;
	stack<int> numParent;
	int num = 0;
	...
	while (true)
	{
		if (tempNode == NULL)
		{
			...
			while (num == tempNode->parents.size() - 1)
			{
				s.pop();
				numParent.pop();
			...
			numParent.pop();
			numParent.push(num);
			tempNode = tempNode->parents[num];
		}
		else
		{
			partialPath.push_back(tempNode->name);
			s.push(tempNode);
			numParent.push(0);
...
public:
	vector<vector<string>> findLadders(string beginWord, string endWord, vector<string>& wordList) {
		...
		map<int, Node*> numToNode;

		Node* empty = NULL;

		Node* beginNode = new Node();
		beginNode->name = beginWord;
		beginNode->parents.push_back(empty);

		//广度优先搜索
		vector<vector<string>> result;

		vector<Node*> wordTree;
		wordTree.push_back(empty);
		wordTree.push_back(beginNode);

		//结束标志，表示已经找到endWord所在的那一层
		//那么将empty标志前的查找完，就可以结束了
		int exit = 0;

		while (true)
		{
			if (wordTree.front() == NULL)
			{
				...
				if (wordTree.size() == 0 || exit == 1)
				{
					for (int i = 0; i < result.size(); ++i)
					{
						...
						{
							result[i].push_back(result[i][j]);
						}
						result[i].erase(result[i].begin(), result[i].begin() + size);
						result[i].push_back(endWord);
					}
			...
			else
			{
				...
				if (search(wordTree.front(), endWord, wordList, flag, wordTree, flagNum, numToNode))
				{
					traverse(wordTree.front(), result);
					exit = 1;
				}
				wordTree.erase(wordTree.begin());
			}
